...
function Fmt fstage2(Bit#(`xlen) hartid 
`ifdef DUAL_ISSUE
, Bit#(1) buffer_no
`endif
, FwdType op1, Op1type op1type, FwdType op2, Op2type op2type, 
                        FwdType op3, Instruction_type insttype, Stage3Meta meta, Bit#(`xlen) mtval );
Fmt result = $format("[%2d]STAGE2 : ", hartid
`ifdef DUAL_ISSUE
  ) + $format(" BUFFER_NO: ", buffer_no
`endif
);
  Fmt op1_addr = ?;
...
`endif
  if (op2type == Immediate)
    op2_addr = $format(" RS2=") + op2_addr + $format("Immediate[%h]" 
`ifdef DUAL_ISSUE
,op2.addr
`endif
,op2.data);
