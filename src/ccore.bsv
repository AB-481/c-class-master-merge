...
module mkccore_axi4#(Bit#(`vaddr) resetpc, parameter Bit#(`xlen) hartid 
`ifdef testmode 
,Bool test_mode 
`endif 
)(Ifc_ccore_axi4);
...
`ifdef dcache
  ...
  * only when rg_burst_count == 0, else the requests are stalled. */
  ...
  Reg#(Bit#(TLog#(TMul#(TMul#(`dwords, 8), `dblocks)))) rg_shift_amount <- mkReg(
    `ifdef DUAL_ISSUE 
  `dwords * 8 
  `else 
  `buswidth 
...
`endif
  ...
  * the fabric*/
  rule rl_handle_imem_line_request;
    ...
    , 0 
    `else
    , `axi4_id_width, `USERSPACE
  `endif
    ) imem_request = AXI4_Rd_Addr {araddr : truncate(request.address),
...
`endif	
    ...
    // if read operation send transaction on the i_rd_addr channel
      ...
      ,`axi4_id_width, `USERSPACE
    `endif
      ) dmem_request = AXI4_Rd_Addr {araddr : truncate(req.address), aruser: ?,
      ...
      ,`axi4_id_width, `USERSPACE
    `endif
      ) aw = AXI4_Wr_Addr {awaddr : truncate(req.address), awuser : 0,
...
`ifdef dcache
  ...
  Reg#(Maybe#(AXI4_Rd_Addr#(`paddr 
  `ifdef DUAL_ISSUE 
  ,0 
  `else 
  ,`axi4_id_width, `USERSPACE
  `endif
  ))) rg_read_line_req <- mkReg(tagged Invalid);
...
`endif
  ...
  The constraint is fulfilled using the register (wr in dual issue) rg_write_req which holds the current address of
  ...
  */
  ...
  `endif
    ...
    if(
      `ifdef DUAL_ISSUE
      wr_write_req matches tagged Valid .waddr 
      `else 
      rg_write_req matches tagged Valid .waddr 
      `endif
      ) begin
